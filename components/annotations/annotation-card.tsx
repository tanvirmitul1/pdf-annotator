...
import { Trash2, X, AlignCenter, AlignLeft, AlignRight } from "lucide-react"
...
export function AnnotationCard({ documentId, annotation, isExpanded, collaborators }: AnnotationCardProps) {
  ...
  async function updateAnnotationProperty(updates: Partial<PositionData>) {
    if (!annotation || !currentUser) return
    if (!isAnnotationOwner(annotation, currentUser.id)) return
    ...
    try {
      ...
      updateAnnotation({
        id: annotation.id,
        documentId,
        positionData: nextPositionData,
      })

      pushUndo({
        action: "update",
        before: annotation,
        after: { ...annotation, positionData: nextPositionData } as AnnotationWithTags,
      })
  ...
  const pos = annotation.positionData as PositionData & {
    opacity?: number
    strokeWidth?: number
    fillColor?: string
    fontFamily?: string
  ...
  return (
    <div
      ref={panelRef}
      className={cn(
        "flex w-full flex-col overflow-hidden bg-card/40 backdrop-blur-3xl transition-all duration-300 ease-in-out border rounded-lg shadow-sm hover:border-primary/30",
        isExpanded ? "ring-1 ring-primary/50" : "cursor-pointer"
      )}
      onClick={() => {
        if (!isExpanded) {
    ...
    >
      <div className="flex shrink-0 items-center gap-2 border-b border-border/40 px-3 py-2 bg-muted/20">
        ...
        {isExpanded && (
          <button
            ...
            onClick={(e) => { e.stopPropagation(); closeAnnotation(); }}
            className="ml-auto flex size-6 items-center justify-center rounded-md text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
          >
      ...
      <div className="space-y-4 p-4">
          ...
          <p className="text-center text-[9px] text-muted-foreground/40 font-medium pt-2 border-t mt-4">
            ...
            <div className="space-y-4 mt-4 animate-in fade-in slide-in-from-top-2">
              ...
              <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  ...
                  {["RECTANGLE", "CIRCLE", "CLOUD", "TEXTBOX"].includes(annotation.type) && (
                    <div className="flex flex-col gap-1.5">
                      <span className="text-[9px] font-black uppercase tracking-wider text-muted-foreground/60">Fill</span>
                      <ColorPicker
                        value={pos.fillColor ?? "transparent"}
                        onChange={(fillColor) => updateAnnotationProperty({ fillColor })}
                        size="sm"
                        showTransparent
                      />
                    </div>
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-center justify-between">
                    <span className="text-[9px] font-black uppercase tracking-wider text-muted-foreground/60">Opacity</span>
                  ...
                  <input
                    ...
                    step="0.01"
                    value={pos.opacity ?? 1}
                    onChange={(e) => updateAnnotationProperty({ opacity: parseFloat(e.target.value) })}
                    className="w-full h-1.5 bg-primary/10 rounded-full appearance-none cursor-pointer accent-primary"
                  />
                </div>

                {/* Thickness for shapes */}
                {["RECTANGLE", "CIRCLE", "ARROW", "LINE", "CLOUD", "FREEHAND"].includes(annotation.type) && (
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] font-black uppercase tracking-wider text-muted-foreground/60">Thickness</span>
                      <span className="text-[10px] font-mono font-bold text-primary/80">
                        {pos.strokeWidth ?? 2}px
                      </span>
                    </div>
                    <input
                      type="range"
                      ...
                      max="20"
                      step="1"
                      value={pos.strokeWidth ?? 2}
                      onChange={(e) => updateAnnotationProperty({ strokeWidth: parseInt(e.target.value) })}
                      className="w-full h-1.5 bg-primary/10 rounded-full appearance-none cursor-pointer accent-primary"
                    />
                  </div>
                )}

                {/* Text Formatting for Textboxes */}
                {annotation.type === "TEXTBOX" && (
                  <div className="space-y-4">
                    <div className="flex flex-col gap-1.5">
                      <span className="text-[9px] font-black uppercase tracking-wider text-muted-foreground/60">Font</span>
                      <div className="flex items-center gap-1 bg-accent/20 p-1 rounded-lg">
                        {FONTS.map((font) => (
                          <button
                            key={font.value}
                            onClick={() => updateAnnotationProperty({ fontFamily: font.value })}
                            className={cn(
                              "flex-1 px-2 py-1 text-[10px] font-medium rounded-md transition-all",
                              pos.fontFamily === font.value 
                    ...
                    <div className="flex flex-col gap-1.5">
                      <span className="text-[9px] font-black uppercase tracking-wider text-muted-foreground/60">Size & Align</span>
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-1 bg-accent/20 p-1 rounded-lg">
                          {FONT_SIZES.map((size) => (
                            <button
                              key={size.value}
                              onClick={() => updateAnnotationProperty({ fontSize: size.value })}
                              className={cn(
                                "size-6 flex items-center justify-center text-[10px] font-bold rounded-md transition-all",
                                (pos.fontSize ?? 16) === size.value 
                                  ? "bg-background text-foreground shadow-sm ring-1 ring-border/50" 
                                  : "text-muted-foreground hover:text-foreground"
                              )}
                            >
                        ...
                        <div className="flex items-center gap-1 bg-accent/20 p-1 rounded-lg">
                          <button 
                            onClick={() => updateAnnotationProperty({ textAlign: "left" })}
                            className={cn("p-1.5 rounded-md transition-all", (pos.textAlign ?? "left") === "left" ? "bg-background text-foreground shadow-sm ring-1 ring-border/50" : "text-muted-foreground hover:text-foreground")}
                          >
                            <AlignLeft className="size-3" />
                          </button>
                          <button 
                            onClick={() => updateAnnotationProperty({ textAlign: "center" })}
                            className={cn("p-1.5 rounded-md transition-all", pos.textAlign === "center" ? "bg-background text-foreground shadow-sm ring-1 ring-border/50" : "text-muted-foreground hover:text-foreground")}
                          >
                            <AlignCenter className="size-3" />
                          </button>
                          <button 
                            onClick={() => updateAnnotationProperty({ textAlign: "right" })}
                            className={cn("p-1.5 rounded-md transition-all", pos.textAlign === "right" ? "bg-background text-foreground shadow-sm ring-1 ring-border/50" : "text-muted-foreground hover:text-foreground")}
                          >
                ...
                {isEditingDescription ? (
                  <div className="space-y-2.5">
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" className="h-7 px-3 text-[10px] font-bold rounded-lg" onClick={(e) => { e.stopPropagation(); void triggerCommentSave(comment); setIsEditingDescription(false); }}>
                        Save
                      </Button>
                      <Button size="sm" variant="ghost" className="h-7 px-3 text-[10px] font-bold rounded-lg" onClick={(e) => { e.stopPropagation(); setComment(annotation?.content ?? ""); setIsEditingDescription(false); }}>
                        Cancel
                      </Button>
                    </div>
                  </div>
